...
public class Joystick : MonoBehaviour {
    ...
    {
        // 操作関連イベントを登録
        InputManager.Instance.OnTouchBegin.AddListener(() => { this.OnBeginPress(); });
        InputManager.Instance.OnTouchEnd.AddListener(() => { this.OnEndPress(); });
        InputManager.Instance.OnTouchMove.AddListener(() => { this.OnDrag(); });
    }
    ...
    private Vector3 GetTouchPointInWorld ()
    {
        ...
        Vector2 tocuhPos = InputManager.Instance.GetPrevPos();
        Vector2 screenPos = Vector2.zero;
        RectTransformUtility.ScreenPointToLocalPointInRectangle(GetComponent<RectTransform>(),
          new Vector2(tocuhPos.x, tocuhPos.y),
          null,
          out screenPos
        );
